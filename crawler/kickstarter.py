...
def get_kickstarter_ideas_with_page_index(
    ...
    url = f"https://www.kickstarter.com/discover/advanced?state=live&category_id={sub_category_id}&raised=2&sort=most_backed&page={page}"
    if seed:
        url += f"&seed={seed}"
    try:
        html_content = _get_html_content(url)
        ...
        for i, project_div in enumerate(project_divs):
            ...
            idea_result: IdeaResult = {
                ...
                "众筹金额": project_data_dict.get("converted_pledged_amount"),
                "众筹人数": project_data_dict.get("backers_count"),
